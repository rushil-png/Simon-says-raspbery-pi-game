...
The length of the sequence increases at each new level. The project also includes a web version that allows the game to be played in a browser without requiring Raspberry Pi hardware.
...
How the Game Works
The game shows a sequence of LED flashes
The player must repeat the same sequence using buttons
Each round adds new elements to the sequence
...
How to Run
Raspberry Pi (GPIO mode)
Connect the Raspberry Pi with all components correctly wired
Ensure the GPIO pins match the configuration in the code
Run the program:
python3 SIMON.py
Web Version
